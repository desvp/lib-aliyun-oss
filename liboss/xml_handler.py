...
class ErrorXml:
    ...
    def show(self):
        _template = 'Code: %s\nMessage: %s\nResource: %s\nRequestId: %s \nHostId: %s'
        _data = (self.code, self.msg, self.resource, self.request_id, self.host_id)
        print(_template % _data)
...
class Content:
    ...
    def show(self):
        _template = 'Key: %s\nLastModified: %s\nETag: %s\nSize: %s\nStorageClass: %s'
        _data = (self.key, self.last_modified, self.etag, self.size, self.storage_class)
        print(_template % _data)
        self.owner.show()
...
class Part:
    ...
    def show(self):
        _template = 'PartNumber: %s\nPartName: %s\nPartSize: %s\nETag: %s\n'
        _data = (self.part_num, self.object_name, self.object_size, self.etag)
        print(_template % _data)
...
class PostObjectGroupXml:
    ...
    def show(self):
        _template = 'Post Object Group, Bucket: %s\nKey: %s\nSize: %s\nETag: %s'
        _data = (self.bucket, self.key, self.size, self.etag)
        print(_template % _data)
...
class GetObjectGroupIndexXml:
    ...
    def show(self):
        _template = 'Bucket: %s\nObject: %s\nEtag: %s\nObjectSize: %s'
        _data = (self.bucket, self.key, self.etag, self.file_length)
        print(_template % _data)
        print('\nPart list:')
...
class GetObjectLinkInfoXml:
    ...
    def show(self):
        _template = 'Bucket: %s\nType: %s\nObject: %s\nEtag: %s\nLastModified: %s'
        _data = (self.bucket, self.type, self.key, self.etag, self.last_modified)
        print(_template % _data)
        print('\nPart list:')
        for p in self.index_list:
            p.show()
...
class GetBucketXml:
    ...
    def show(self):
        _template = 'Name: %s\nPrefix: %s\nMarker: %s\nNextMarker: %s\n'
        _template += 'MaxKeys: %s\nDelimiter: %s\nIsTruncated: %s'
        _data = (self.name, self.prefix, self.marker, self.nextmarker,
                 self.maxkeys, self.delimiter, self.is_truncated)
        print(_template % _data)
        print('\nPrefix list:')
...
class LifecycleRule:
    ...
    def show(self):
        _template = 'ID: %s\nPrefix: %s\nStatus: %s\nExpiration: %s\n'
        _data = (self.id, self.prefix, self.status, self.expiration)
        print(_template % _data)
...
class GetObjectInfoXml:
    ...
    def show(self):
        _template = 'Bucket: %s\nType: %s\nKey: %s\nLastModified: %s\nETag: %s\nContent-Type: %s\nSize: %s'
        _data = (self.bucket, self.type, self.key, self.last_modified, self.etag, self.content_type, self.size)
        print(_template % data)


# vim:nocp:tw=120:sw=4:ts=4:
